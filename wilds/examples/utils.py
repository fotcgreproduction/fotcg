...
def update_average(prev_avg, prev_counts, curr_avg, curr_counts, device=None):
    ...
    if isinstance(prev_avg, torch.Tensor) or isinstance(curr_avg, torch.Tensor):
        prev_weight = prev_weight.to(device)
        curr_weight = curr_weight.to(device)
        prev_avg = prev_avg.to(device)
        curr_avg = curr_avg.to(device)

        

    '''# ????????
    print(f'prev_weight: {type(prev_weight)}')
    print(f'prev_counts: {type(prev_counts)}')
    print(f'curr_weight: {type(curr_weight)}')
    print(f'prev_avg: {type(prev_avg)}')
    print(f'curr_avg: {type(curr_avg)}')

    try:
        print(f'prev_weight: {prev_weight.device}')
        print(f'prev_counts: {prev_counts.device}')
        print(f'curr_weight: {curr_weight.device}')
        print(f'prev_avg: {prev_avg.device}')
        print(f'curr_avg: {curr_avg.device}')

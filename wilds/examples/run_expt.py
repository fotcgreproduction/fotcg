...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="WILDS unlabeled dataset version number.",
    )
    parser.add_argument(
        "--use_unlabeled_y",
        default=False,
    ...
    parser.add_argument(
        "--unlabeled_loader_kwargs", nargs="*", action=ParseKwargs, default={}
    )
    ...
    parser.add_argument(
        "--use_hafn", default=False, type=parse_bool, const=True, nargs="?"
    )
    ...
    parser.add_argument(
        "--soft_pseudolabels", default=False, type=parse_bool, const=True, nargs="?"
    )
    parser.add_argument("--algo_log_metric")
    parser.add_argument(
        "--process_pseudolabels_function",
        choices=supported.process_pseudolabels_functions,
    )
    ...
    parser.add_argument(
        "--val_metric_decreasing", type=parse_bool, const=True, nargs="?"
    )
    ...
    parser.add_argument(
        "--scheduler_metric_split", choices=["train", "val"], default="val"
    )
    parser.add_argument("--scheduler_metric_name")

    # Evaluation
    parser.add_argument(
        "--process_outputs_function", choices=supported.process_outputs_functions
    )
    parser.add_argument(
        "--evaluate_all_splits", type=parse_bool, const=True, nargs="?", default=True
    )
    ...
    parser.add_argument(
        "--save_best", type=parse_bool, const=True, nargs="?", default=True
    )
    parser.add_argument(
        "--save_last", type=parse_bool, const=True, nargs="?", default=True
    )
    parser.add_argument(
        "--save_pred", type=parse_bool, const=True, nargs="?", default=True
    )
    ...
    # Data
    # NEW: Own dataset
    if config.dataset == "spu_mnist":
        from datasets.spu_mnist_dataset import SpuMNISTDataset

        full_dataset = SpuMNISTDataset(
        ...
        from datasets.cmnist_debug_dataset import CMNISTDDataset

        full_dataset = CMNISTDDataset(
            root_dir=config.root_dir,
            download=config.download,
            split_scheme=config.split_scheme,
            **config.dataset_kwargs,
    ...
    elif config.dataset == "n_mnist":
        from datasets.noisy_mnist_dataset import NMNISTDataset

        full_dataset = NMNISTDataset(
            root_dir=config.root_dir,
            download=config.download,
            split_scheme=config.split_scheme,
            **config.dataset_kwargs,
        )
    elif config.dataset == "r_mnist":
        from datasets.rot_mnist_dataset import RMNISTDataset

        full_dataset = RMNISTDataset(
            root_dir=config.root_dir,
            download=config.download,
            split_scheme=config.split_scheme,
            **config.dataset_kwargs,
    ...
    elif config.dataset == "n_simple":
        from datasets.noisy_simple_dataset import NoisySimpleDataset

        full_dataset = NoisySimpleDataset(
            root_dir=config.root_dir,
            download=config.download,
            split_scheme=config.split_scheme,
            **config.dataset_kwargs,
    ...
    # New: has issues, so not working
    elif config.dataset == "multinli":
        from datasets.multinli_dataset import MultiNLIDataset

        full_dataset = MultiNLIDataset(
            root_dir=config.root_dir,
    ...
    if config.unlabeled_split is not None:
        ...
        if config.algorithm == "NoisyStudent":
            ...
            teacher_outputs = infer_predictions(
                teacher_model, sequential_loader, config
            )
        ...
        train_grouper = CombinatorialGrouper(
            dataset=full_dataset, groupby_fields=config.groupby_fields
        )
    ...
    if (
        config.no_group_logging
        and full_dataset.is_classification
        and full_dataset.y_size == 1
        and full_dataset.n_classes <= 10
    ):
        log_grouper = CombinatorialGrouper(dataset=full_dataset, groupby_fields=["y"])
    ...
    if not config.eval_only:
        ...
        if resume:
            ...
            try:
                prev_epoch, best_val_metric = load(
                    algorithm, save_path, device=config.device
                )
        ...
        if config.gradient_accumulation_steps > 1:
            logger.write(
                ...
                + (
                    ...
                    if unlabeled_dataset and config.unlabeled_batch_size
                    else ""
                )
        ...
        best_epoch, best_val_metric = load(
            algorithm, eval_model_path, device=config.device
        )
        if config.eval_epoch is None:
            epoch = best_epoch
        else:

...
class CG(SingleModelAlgorithm):
    def __init__(
        ...
        # step size
        # self.rho = config.metacg_rho #TODO IS it really not used?
        self.outer_lr = config.lr

        self.C = config.cg_C
        _, self.g_counts = group_info
        self.g_counts = torch.tensor(
            self.g_counts, device=self.device, dtype=torch.float32
        )
        ...
        self.alpha = torch.autograd.Variable(
            torch.ones(self.num_groups, device=self.device) * (1.0 / self.num_groups),
            requires_grad=True,
        )
        self.lmbda = torch.autograd.Variable(
            torch.zeros(self.num_groups, device=self.device), requires_grad=True
        )
    ...
    # ADDED BY US
    # Added optional kwarg unlabeled_batch=None to be in line with single_model_algorithm.py
    def process_batch(self, batch, unlabeled_batch=None):
        ...
        """
        results = super().process_batch(batch, unlabeled_batch)
    ...
    # ADDED BY US
    # Added optional kwarg should_step=False to be in line with single_model_algorithm.py
    def _update(self, results, should_step=True):
        ...
        for li in range(self.num_groups):
            all_grads[li] = torch.autograd.grad(
                group_losses[li], params, retain_graph=True
            )
        ...
        for li in range(self.num_groups):
            for lj in range(self.num_groups):
                ...
                RTG[li, lj] = dp / torch.clamp(
                    torch.sqrt(vec1_sqnorm * vec2_sqnorm), min=1e-3
                )

        _gl = torch.sqrt(group_losses.detach().unsqueeze(-1))
        RTG = torch.mm(_gl, _gl.t()) * RTG
        # print(f'step size: {self.step_size}; RTG: {RTG.shape}, wts: {self.wts.shape}') #TODO
        ...
        # update model
        super()._update(results, should_step)

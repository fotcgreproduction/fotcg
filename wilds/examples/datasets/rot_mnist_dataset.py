...
def make_environment(images, labels, fracs):
    ...
    gs = torch.tensor(gs)
    return {"X": (X.float()), "y": Y[:, None], "g": gs}


class RMNISTDataset(WILDSDataset):
    def __init__(self, root_dir="data", download=False, split_scheme="official"):
        ...
        self._dataset_name = "mnist"  # NEW, was "rmnist" before
        ...
        train_data = make_environment(
            mnist_train[0], mnist_train[1], [0.49, 0.49, 0.02]
        )
        ...
        for di, d in enumerate([train_data, val_data, test_data]):
            ...
            for j in range(len(y)):
                _x_array.append(
                    np.transpose(x[j].numpy(), axes=[2, 1, 0]).astype(np.uint8)
                )
                _y_array.append(y[j])
                _g_array.append(g[j])
            _split_array += [di] * len(y)

        _y_array = np.array(_y_array, dtype=np.float64)  # NEW: convert to float64
        ...
        self._metadata_array = torch.stack(
            (torch.LongTensor(_g_array), self._y_array), dim=1
        )
        self._metadata_fields = ["group", "y"]
        self._metadata_map = {
            "group": [" rot0", " rot1", " rot2"],
            "y": ["%d" for d in range(10)],
        }

        self._original_resolution = (28, 28)

        self._eval_grouper = CombinatorialGrouper(
            dataset=self, groupby_fields=(["group"])
        )
    ...
    def get_input(self, idx):
        ...
        pil_img = Image.fromarray(self._input_array[idx]).convert("RGB").rotate(gi * 30)
        return pil_img
...
if __name__ == "__main__":
    dset = RMNISTDataset("data")
    train, val, test = (
        dset.get_subset("train"),
        dset.get_subset("val"),
        dset.get_subset("test"),
    )
